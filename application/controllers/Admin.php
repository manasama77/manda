...
class Admin extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['total_data_daily']           = number_format($this->form_data_model->count_trackid_daily($tgl_obj->format('Y-m-d')), 0, ',', '.');
		$data['total_data_daily_valid']     = $this->form_data_model->count_trackid_daily_valid($tgl_obj->format('Y-m-d'));
		$data['total_data_daily_invalid']   = $data['total_data_daily'] - $data['total_data_daily_valid'];
		$data['percent_data_daily_valid']   = $this->generate_valid_percentage($data['total_data_daily'], $data['total_data_daily_valid']);
		$data['percent_data_daily_invalid'] = $this->generate_valid_percentage($data['total_data_daily'], $data['total_data_daily_invalid']);

		$data_trackid_daily = $this->form_data_model->get_trackid_daily($tgl_obj->format('Y-m-d'));

		for ($i = 1; $i <= 7; $i++) {
			$data['treg_' . $i . '_all']             = $this->form_data_model->count_trackid_all($data_trackid_daily, "Treg-" . $i);
			$data['treg_' . $i . '_valid']           = $this->form_data_model->count_trackid_valid($data_trackid_daily, "Treg-" . $i);
			$data['treg_' . $i . '_invalid']         = $data['treg_' . $i . '_all'] - $data['treg_' . $i . '_valid'];
			$data['treg_' . $i . '_valid_percent']   = $this->generate_valid_percentage($data['treg_' . $i . '_all'], $data['treg_' . $i . '_valid']);
			$data['treg_' . $i . '_invalid_percent'] = $this->generate_valid_percentage($data['treg_' . $i . '_all'], $data['treg_' . $i . '_invalid']);
		}

		$counter = 6;
		for ($i = 1; $i <= 16; $i++) {
			${'from_time_' . $i} = $counter . ':00:00';
			$counter++;
			${'to_time_' . $i} = $counter . ':00:00';

			$data['interval' . $i] = $this->form_data_model->count_date_based_range_time($data_trackid_daily, ${'from_time_' . $i}, ${'to_time_' . $i});
			$data['ao' . $i]    = $this->form_data_model->count_ao_based_range_time($data_trackid_daily, ${'from_time_' . $i}, ${'to_time_' . $i});
		}
		...
		$this->load->view('admin/index_vitamin', $data);
	}
	...
	public function index2()
	{
		...
		$this->load->view('templates/sidebar', $data);
		$this->load->view('templates/topbar', $data);
		$this->load->view('admin/index_bck', $data);
		$this->load->view('templates/footer');
	}
	...
	public function generate_valid_percentage($all, $valid)
	{
		return ($all == 0 || $valid == 0) ? 0 : round(($valid / $all) * 100, 1);
	}

	public function get_yearly_data()
	{
		$tgl_obj = new DateTime('now');
		$data = $this->form_data_model->count_trackid_yearly($tgl_obj->format('Y'));
		echo json_encode(['data' => number_format($data, 0, ",", ".")]);
	}

	public function get_monthly_data()
	{
		$tgl_obj = new DateTime('now');
		$data = $this->form_data_model->count_trackid_montly($tgl_obj->format('Y'), $tgl_obj->format('m'));
		echo json_encode(['data' => number_format($data, 0, ",", ".")]);
	}
}
